...
#' For large experiments, however, the average variability within blocks may be too large for good precision of estimation and then it may 
#' be desirable to sub-divide large complete blocks into smaller incomplete blocks. Sometimes it is advantageous to use a double 
#' blocking system in which one set of blocks, usually called row blocks, is crossed with a second set of blocks, usually called column blocks. Double
...
#' 
#' The \code{blocksdesign} package provides for the construction of general block designs with simple or crossed blocks that can be nested repeatedly 
...
#' \code{blocks} includes a data frame of the block and treatment factors for each plot and a table showing the achieved D- and A-efficiency factors
