...
Block designs group experimental units into homogeneous blocks to provide the maximum precision for treatment comparisons. The most basic type of blocks are complete randomised blocks where each block contains one or more complete sets of treatments. Complete blocks estimate all treatment effects with maximum efficiency and provide excellent designs for small or medium size experiments. However, for large designs, they may be too large to provide homogeneous comparisons within the same block.

One method of reducing the variability of large complete blocks is to subdivide each complete main block into smaller incomplete blocks that are fully nested within the individual main blocks. One of the earliest examples of an incomplete block design was given by Yates (1936) who introduced the class of designs called resolvable balanced lattices. These designs exist for equi-replicate treatments if the number of treatments is the square of a prime  or prime power v, the nested block size is v and the number of replications is not more than v+1. For 3 or fewer replicates, lattice designs exist for any square number of treatments v\*v where the block size is v. Although balanced lattice designs have a high degree of balance and are highly efficient, their limited availability make them of limited practical value.

Following the development of resolvable balanced lattice designs, incomplete block designs were generalized in a number of ways and many other classes of incomplete block designs were introduced (see Cochran and Cox 1957). A major limitation on these early designs was that results had to be hand-calculated and some of the early designs, such as the partially balanced incomplete block designs (PBIB) of Clatworthy (1956), were developed specifically to simplify this analysis. However, it is now known that some PBIB designs are very inefficient for certain treatment comparisons therefore such designs are not now much used. Other developments included cyclic designs for incomplete blocks, see John (1966) and John and Williams (1995). 

### Software

Incomplete block designs improve the within-blocks precision of treatment comparisons but they also confound treatment information between blocks. An efficient analysis must take full account of all the different sources of variability in a design and before the development of modern computers and modern software, the complexity of the analysis of incomplete block designs was a major restriction on the choice of design. Nowadays the availability of modern computer analysis methods such as the mixed model estimation methods available in the R packages 'lme4' (Bates et al 2014) and 'lmerTest' (Kuznetsova et al 2014) have largely removed this restriction. Modern computer software is capable of routine analysis of very general block designs and there is now no computational limitation on the use of very general block designs.

### Algorithms

Computer algorithms for efficient block design have been extensively researched and Cook and Nachtsheim (1980) gave a review of work up to about 1980. One important development was the use of swapping algorithms for the construction of efficient block designs for arbitrary number of treatments, arbitrary replication and arbitrary block sizes. Swapping algorithms for nested blocks work by swapping treatments at random between sets of nested blocks contained within existing blocks. Improving swaps are retained and the process is repeated until no further improvement is possible. A good general purpose swapping algorithm will give a good design close to optimal for any required design and any chosen optimality criteria. There are a number of possible choices of optimality criteria available but the most commonly used criteria are probably A-optimality, which maximizes the harmonic mean, and D-optimality, which maximizes the geometric mean of the eigenvalues of the design information matrix. Usually, the two criteria give similar though not identical results. A-optimality may have a more direct interpretation than D-optimality (John and Williams 1995 Chapter 2) but D-optimality seems to be more tractable for manipulation and optimization. 
...
In a multi-stratum nested blocks design, the choice of the smaller blocks is constrained by the fixed treatment allocation of the larger blocks meaning that there could be some loss of efficiency on the smaller blocks due to the constraints of the larger blocks. Our experience is that for medium to large designs any loss of efficiency due to multi-stratum nesting is negligible. In the Patterson and Hunter example, the upper efficiency bound for blocks of size 4 is .69012 and the best achieved efficiency for blocks of size 4 in a 'classical' resolvable incomplete block design using the 'blocksdesign' package, was .68804 whereas the best achieved efficiency for the multi-stratum nested blocks of size 4 was .6867. Thus the loss of efficiency on blocks of size 4 in the multi-stratum nested design compared with blocks of size 4 in the 'conventional' single startum nested design was less than 0.2%. 

As a further example, a design for two replicates of 256 treatments was optimized assuming a maximal multi-stratum nesting scheme based on recursive 2-level nesting down to the minimum possible block size. The following table compares the achieved efficiencies of each blocks stratum in a repeatedly nested multi-stratum design relative to the achieved efficiencies when fitted as single nested strata in 'classical' nested block designs with resolvable main blocks. The final column shows upper bounds for each block size in completely unconstrained block designs. 
   			
  
Block numbers  | Multi-stratum model | Separate models | Upper bounds|
---------------| --------------------|-----------------|-------------|
2              | 1                   | 1               |1            |
...
In this example, there was virtually no loss of efficiency in any stratum due to the constraints of the multi-stratum model.
